<li data-id="{{ $menu->id }}" class="menu_item {{ $args['li_class'] ?? '' }} {{ $menu->custom_class }}">
    @if (count($menu->children) > 0)
        <a href="javascript:void(0)" rel="nofollow" class="menu-link {{ $args['a_class'] }} ripple has-dropdown"
            data-target="menu-dropdown{{ $menu->id }}" data-align="left-bottom">
        @else
            <a href="{{ generate_menu_url($menu->url) }}" class="menu-link {{ $args['a_class'] ?? '' }} ripple"
                target="{{ $menu->target }}">
    @endif
