        ...
        @if ($follows->count() > 0)
            <div class="profile-section follow-images ">
                @foreach ($follows as $follow)
                    @if ($follow->follower)
                        <a class="follow-image big" href="{{ $follow->follower->profile_link }}"
                            title="{{ $follow->follower->username }}">
                            <img src="{{ makepreview($follow->follower->icon, 's', 'members/avatar') }}" width="90"
                                height="90" alt="{{ $follow->follower->username }}">
