<div class="box">
    {!! Form::open([
        'action' => isset($menu_item->id) ? 'Admin\MenuItemController@update' : 'Admin\MenuItemController@store',
        'method' => isset($menu_item->id) ? 'PUT' : 'POST',
    ]) !!}
    {{ csrf_field() }}
    ...
    <div class="box-body">
        <div class="form-group">
            ...
            {!! Form::text('title', isset($menu_item->title) ? $menu_item->title : old('title'), [
        ...
        <div class="form-group">
            ...
            {!! Form::text('url', isset($menu_item->url) ? $menu_item->url : old('url'), [
        ...
        <div class="form-group">
            ...
            {!! Form::select(
                'target',
                ['_self' => trans('v4.same_tab'), '_blank' => trans('v4.new_tab')],
                isset($menu_item->target) ? $menu_item->target : old('target'),
                [
        ...
        <div class="form-group">
            ...
            {!! Form::text('icon', isset($menu_item->icon) ? $menu_item->icon : old('icon'), [
        ...
        <div class="form-group">
            ...
            {!! Form::text(
                'custom_class',
                isset($menu_item->custom_class) ? $menu_item->custom_class : old('custom_class'),
                ['id' => 'add_menu_item_custom_class', 'class' => 'form-control input-field mb-2'],
            ) !!}
        </div>
        @if (get_multilanguage_enabled())
            <div class="form-group">
                ...
                {!! Form::select(
                    'language',
                    get_buzzy_language_list_options(),
                    !empty($menu_item->language) ? $menu_item->language : get_buzzy_locale(),
                    [
                        'id' => 'changeLanguage',
                        'id' => 'add_menu_item_language',
                        'class' => 'form-control input-field mb-2',
                    ],
                ) !!}
            </div>
        ...
        <div>
            ...
            @if (isset($menu_item->id))
                <a href="{{ route('admin.menu.show', ['menu' => $menu->id]) }}"
                    class="btn btn-default pull-right">{{ trans('admin.Cancel') }}</a>
            @endif
        </div>
    </div>
    {!! Form::close() !!}
...
<div class="box">
    <div class="box-body">
        {!! Form::open(['action' => 'Admin\MenuItemController@storeFromCategory', 'method' => 'POST']) !!}
        <div class="form-group">
            ...
            <select class="form-control" name="category_id">
                @php($categories = \App\Category::byMain()->byLanguage(get_buzzy_locale())->get())
                @foreach ($categories as $alt_category)
                    <optgroup>
                        <option value="{{ $alt_category->id }}">
                            {{ $alt_category->name }}
                        </option>
                        @foreach ($alt_category->children()->orderBy('name')->get() as $altalt_category)
                            <option value="{{ $altalt_category->id }}">
                                {{ $alt_category->name }} / {{ $altalt_category->name }}
                            </option>
                            @foreach ($altalt_category->children()->orderBy('name')->get() as $altaltalt_category)
                                <option value="{{ $altaltalt_category->id }}">
                                    {{ $alt_category->name }} / {{ $altalt_category->name }} /
                                    {{ $altaltalt_category->name }}
                                </option>
                            @endforeach
                        @endforeach
                    </optgroup>
