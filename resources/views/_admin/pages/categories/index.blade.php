...
@section('content')
    ...
    <section class="content">
        <div class="row">
            <div class="col-md-4">
                @include('_admin.pages.categories.particles._form', ['categories' => $categories])
            </div><!-- /.col -->
                        ...
                        <div class="nav-tabs-custom category-list">
                            <ul class="nav nav-tabs pull-right">
                                <li class="pull-left header">
                                    ...
                                    <b>{{ $_category->name }}</b>
                                    <span>(<a
                                            href="{{ route('admin.posts', ['type' => 'category', 'category_id' => $_category->id]) }}">{{ __(':count Posts', ['count' => \App\Post::byCategories(get_category_all_childids_recursively($_category))->byApproved()->byPublished()->count()]) }}</a>)</span>
                                    ...
                                    @if (!array_key_exists($_category->type, get_post_types()))
                                        <span class="pull-right badge bg-red" data-toggle="tooltip"
                                            data-original-title="Post type: {{ $_category->type }} deactivated on Plugins. Users can't add {{ $_category->type }} post type">Not
                                            available</span>
                                    @endif
                                </li>
                                <li class="pull-right header">
                                    <a href="{{ route('admin.categories', ['edit' => $_category->id]) }}"
                                        ...
                                        data-original-title="{{ trans('admin.edit') }}"><i class="fa fa-edit"></i>
                                        {{ trans('admin.edit') }}</a>
                                    <a class="btn btn-sm btn-danger permanently"
                                        href="{{ route('admin.category.delete', ['category' => $_category->id]) }}"
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active" id="tab_{{ $ci }}-1">
                                    @include('_admin.pages.categories.particles._list', [
                                        'altcategories' => $_category,
                                    ])
                                </div><!-- /.tab-pane -->
                            </div>
                        </div>
