<div class="box box-widget widget-user widget-plugin">
    ...
    <div class="widget-user-header bg-default">
        ...
        <div class="info">
            {!! $weblink == null
                ? ''
                : '<a href="' . $weblink . '" target="_blank"><i class="fa fa-globe"></i> Web site</a>' !!}
            {!! $docslink == null
                ? ''
                : '<a href="' . $docslink . '" target="_blank"><i class="fa fa-book"></i> ' . trans('admin.Docs') . '</a>' !!}
            @if ($price != 'soon' && !empty($current_version))
                <span class="text-gray">v.{{ $current_version }}</span>
            @endif
        </div>
    </div>
                ...
                @unless ($price == 'soon')
                    @if ($activation_requied)
                        <a href="javascript:;" data-item-id="{{ $item_id }}" data-item-name="{{ $name }}"
                            ...
                            class="btn btn-warning btn-sm pull-left register-item">
                        ...
                        @endif
                    @elseif(!$instaled)
                        <button type="button" class="btn btn-info btn-sm pull-left download-item"
                            data-item-code="{{ $code }}" data-item-id="{{ $item_id }}"
                    ...
                    @elseif($active)
                        <button type="button" class="btn btn-default btn-sm pull-left activate-item acthover">
                            <span class="current show"><i class="fa fa-check mr-5"></i>
                                {{ trans('admin.Activated') }}</span>
                            <span class="hover hide"><i class="fa fa-remove mr-5"></i>
                                {{ trans('admin.Deactivate') }}</span>
                        </button>
                        @if ($settingon)
                            <button type="button" class="btn btn-warning btn-sm pull-left" data-toggle="modal"
        ...
        <div class="modal modal-info" id="modal{{ $code }}">
            <div class="modal-dialog {{ $code == 'homepagebuilder' ? 'full' : '' }}">
                <div class="modal-content">
                    ...
                    @includeIf('_admin._particles.plugin_settings.' . $code)
                    <div class="clearfix"></div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline pull-left"
                            data-dismiss="modal">{!! trans('admin.close') !!}</button>
                        <input type="submit" value="{{ trans('admin.SaveSettings') }}"
