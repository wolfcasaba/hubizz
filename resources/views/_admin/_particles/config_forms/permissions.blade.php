    ...
    <div class="col-sm-12  col-md-8 col-lg-6">
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
                ...
                <hr>
                 <div class="form-group">
                    <label class="control-label">{{__('Email me when a post is held for approve?')}}</label>
                    <div class="controls">
                        {!! Form::select('POSTS_SEND_MAIL_AWAIT_APPROVE', [true => trans("admin.yes"), false => trans("admin.no")],
                        get_buzzy_config('POSTS_SEND_MAIL_AWAIT_APPROVE', true),
                        ['class' => 'form-control']) !!}
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label">{{__('Email user when post is approved?')}}</label>
                    <div class="controls">
                        {!! Form::select('POSTS_SEND_MAIL_APPROVED', [true => trans("admin.yes"), false => trans("admin.no")],
                        get_buzzy_config('POSTS_SEND_MAIL_APPROVED', true),
                        ['class' => 'form-control']) !!}
                    </div>
                </div>

                <div class="form-group">
                    <label class="control-label">{{__('Email user when post is deleted?')}}</label>
                    <div class="controls">
                        {!! Form::select('POSTS_SEND_MAIL_DELETED', [true => trans("admin.yes"), false => trans("admin.no")],
                        get_buzzy_config('POSTS_SEND_MAIL_DELETED', true),
                        ['class' => 'form-control']) !!}
                    </div>
                </div>
            </div>
