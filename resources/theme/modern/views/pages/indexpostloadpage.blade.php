@foreach ($lastFeatures as $k => $item)
            ...
            <div class="content-timeline__link clearfix">
                <div class="content-timeline__media">
                    <figure class="content-timeline__media__image">
                        <a class="clearfix" href="{{ $item->post_link }}" title="{!! $item->title !!}">
                            <img src="{{ url('assets/images/preloader.gif') }}"
                                data-src="{{ makepreview($item->thumb, 's', 'posts') }}" class="lazyload"
            ...
            <ol class="sidebar-mosts sidebar-mosts--readed column_list tree_column">
                @foreach ($lastVideos->slice(0, 3) as $itema)
                    <li class="sidebar-mosts__item ">
                        <a class="sidebar-mosts__item__link" href="{{ $itema->post_link }}"
                            title="{{ $itema->title }}">
                            <figure class="sidebar-mosts__item__body">
                                <div class="sidebar-mosts__item__image">
                                    <img class="sidebar-mosts__item__image__item lazyload"
                                        src="{{ url('assets/images/preloader.gif') }}"
                                        data-src="{{ makepreview($itema->thumb, 's', 'posts') }}"
                                        alt="{{ $itema->title }}">
                                </div>
    ...
    @endif
    @if ($k == 0)
        @include('_particles.widget.ads', [
            'position' => 'Homencolfirst',
            'width' => 'auto',
            'height' => 'auto',
        ])
    @endif
