...
class PostEditorController extends Controller
{
    ...
    private function savePost($inputs, $post, $update = false)
    {
        try {
            ...
            $post->meta_title = isset($inputs['meta_title']) ? $inputs['meta_title'] : null;
            $post->meta_description = isset($inputs['meta_description']) ? $inputs['meta_description'] : null;
            ...
            if ($update) {
                ...
                $post->user_id = $post->user_id ? $post->user_id : Auth::user()->id;
                $post->save();
            }
            ...
            $tagIds = !empty($inputs['tags']) ? collect(explode(',', $inputs['tags']))->map(function ($tagName) use ($post) {
                return Tag::firstOrCreate([
                    'name' => $tagName,
                    'slug' => $this->createPostSlug($tagName, $tagName),
                    'type' => 'post_tag',
                ], [
