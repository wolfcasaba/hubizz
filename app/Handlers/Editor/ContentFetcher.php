...
class ContentFetcher
{
    ...
    public function getBody($html)
    {
        ...
        //$body = $this->getElementsByClassname($html, 'detail-content-inner');
        $body = "";
        ...
        $body = str_replace(array('data-src', 'data-original'), 'src', $body);

        $allowed_tags = array(
            "<a>", "<b>", "<strong>", "<br>", "<img>", "<video>", "<hr>", "<i>",
            "<h2>", "<h3>", "<h4>", "<h5>", "<h6>",
            "<ul>", "<li>", "<ol>", "<p>", "<s>", "<u>",
        ...
        if (class_exists('DOMDocument')) {
            ...
            $body = is_string($body) ? $this->getContent($body, $allowed_tags, $allowed_attr) : '';
        } else {
            $body = is_string($body) ? strip_tags($body, $allowed_tags) : '';
        }
    ...
    public function getElementsByClassname($html, $className)
    {
        if (empty($html)) {
            return $html;
        }
