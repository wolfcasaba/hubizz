...
class MessageThread extends AbstractModel
{
    use SoftDeletes;
     ...
     *
     * @var null|User|\Illuminate\Database\Eloquent\Model
     */
    ...
    {
        return $this->hasMany(MessageParticipant::class, 'thread_id', 'id');
    }
     ...
     *
     * @return null|User|\Illuminate\Database\Eloquent\Model
     */
    ...
    {
        $participantsTable = "message_participants";
    ...
    public function removeParticipant($userId)
    {
        ...
        MessageParticipant::where('thread_id', $this->id)->whereIn('user_id', $userIds)->delete();
    }
    ...
    public function participantsString($userId = null, $columns = ['username'])
    {
        ...
        $participantNames = $this->getConnection()->table('users')
            ->join('message_participants', 'users.id', '=', 'message_participants.user_id')
            ->where('message_participants.thread_id', $this->id)
    ...
    /**
     * Get first participant of the thread.
     *
     * @param int $userId
     *
     * @return bool
     */
    public function firstParticipant($userId)
    {
    ...
    protected function createSelectString($columns)
    {
        ...
        switch ($dbDriver) {
            ...
            case 'sqlite':
                ...
                $selectString = '(' . $tablePrefix . $usersTable . '.' . $columnString . ') as username';
                break;
            case 'sqlsrv':
                $columnString = implode(" + ' ' + " . $tablePrefix . $usersTable . '.', $columns);
                $selectString = '(' . $tablePrefix . $usersTable . '.' . $columnString . ') as username';
            ...
            default:
                $columnString = implode(", ' ', " . $tablePrefix . $usersTable . '.', $columns);
                $selectString = 'concat(' . $tablePrefix . $usersTable . '.' . $columnString . ') as username';
        }
